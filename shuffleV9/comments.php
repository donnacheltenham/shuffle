	...
	<script type="text/javascript">
		...
		function getFeed( rsscontent ) {
			...
			$channel.children().each( function () {
				...
				if ( $( this ).prop( "tagName" ) == "item" ) {
					...
					//Check if article thumb is empty - if it is, set to empty so nothing is displayed//
					if ( (url) == undefined) {
						var thumb = "";
					} else {
						//Catch images smaller that current div width to prevent them pixelating//
						if (imgwidth > 70) {
					...
					//Build variables for title, description and link values//
						if ( $( this ).prop( "tagName" ) == "title" ) {
					...
					//Build htmls string using previous vars for title, desc and link.  Do not inlclude items not found //
					if (title == undefined) {
		...
		$( document ).ready( function () {
			...
			$( document ).on( "click", '.updatecom', function ( event ) {
				...
				//cycle through the comments, if the ID matchs then set textarea to have comment content//
